      ...
      subtests: report.subtests.map((subtest, subtestIndex) => ({
        ...
        fields: subtest.fields
          .map((field, fieldIndex) => {
            const fieldKey = `result-${reportIndex}-${subtestIndex}-${fieldIndex}`;
            const currentResult = formData[fieldKey];
            const initialResult = field.result;

            // Only include the result if it has been changed or explicitly filled
            return currentResult !== initialResult
              ? {
                  fieldName: field.fieldName,
                  units: field.units,
                  referenceRange: field.referenceRange,
                  result: currentResult || "", // Ensure non-null result
                }
              : null; // Exclude unchanged fields
          })
          .filter(Boolean), // Remove null entries
      })),
      reportNotes: formData[`reportNotes-${reportIndex}`] || "", // Include report notes
    }));
    ...
    <div>
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-3 w-full">
                        ...
                        <input
                          ...
                          placeholder={field.results}
                          className="placeholder:text-gray-800 border font-semibold bg-white rounded-md w-full py-3 px-5 outline-none"
                          value={
                            formData[
      ...
      {isSelectOpen && (
        <Successcard
          ...
          para="Test report updated successfully."
          title="Success"
        />
      )}
    </div>
